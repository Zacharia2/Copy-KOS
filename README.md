...
以树作为载体，以图作为树的延伸。用树存储，用图链接和表现。从点或列表 到树 🌲 到图。树结构是最简单实用的存储方式。以树为基础，用树去承载高阶的图。根到叶子和节点的路径、全部叶子或节点就是所有的（笔记、元素）文件；叶子或节点之间的相互连接就是图或者别的数据结构。

思维是网状形态，内容是树状结构，文本则是线性呈现。要管理信息，输出有价值的内容，根问题就是协调网、树、线三种形态。

关于笔记的层次：

1. 元素构成：内部组件结构，笔记文件的内容的记载（笔记包含什么）
2. 元素实体：独立存在形式，笔记本身
3. 元素属性：外部特征标签，笔记的定义字段和元素语义
4. 元素群：多笔记的组织索引。
5. 元素过程：从创建到修订、复用等等。
